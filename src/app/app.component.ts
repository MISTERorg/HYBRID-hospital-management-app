...
import {menuController,AnimationBuilder,createAnimation,MenuI,Animation,} from '@ionic/core';
import { DrawerScreen } from './shared/drawer';
...
export class AppComponent implements AfterViewInit {
  ...
  { name: 'emergency',icon: 'person',url: '/map',},
  { name: 'doctor', icon: 'information-circle', url: '/doc-home' },
  { name: 'admin', icon: 'people', url: '/admin-home' },
  { name: 'accountant', icon: 'star-half', url: '/acc-home' },
  { name: 'About Us', icon: 'accessibility', url: undefined },
  { name: 'Setting', icon: 'settings', url: '/biometrics'},
  ];
  drawerWidth: number;
  rowWidth: number;
  activeTab = 'Home';
  isSplitPane = false; // hide menu button if split pane is enabled (desktop, pad etc.)
  constructor(public platform: Platform, private menu: MenuController,public authService: AuthenticationService) {
  ...
  this.drawerWidth = deviceWidth * 0.85;
  if (deviceWidth > 992) {
  const splitPaneWidth = (deviceWidth * 34) / 100; // default max split pane width is 28% after 992px
  this.rowWidth = splitPaneWidth - 64;
  this.isSplitPane = true;
  } else {
  this.rowWidth = deviceWidth * 0.85 - 64;
